...
![1783997171014](image/write/1783997171014.png)

这些可以直接在react里面使用; [DatePicker 日期选择器 | TDesign](https://tdesign.tencent.com/react/components/date-picker?tab=api) 学会看文档;
...
答：当前后端返回的数据是简单平铺的，不是按月份划分好的，而我们要做的功能是以月为单位的统计

从Redux中拿到数据    数据二次处理                      按月分组逻辑实现

    (useMemo)                          (lodash)

#### useMemo 是什么:

`useMemo` 是 React 内置性能优化 Hook，专门 **缓存数据二次加工后的结果** ：

组件每次重新渲染时，若原始数据没变化，直接复用上次处理好的数据，不再重复执行过滤、排序、映射、聚合等复杂计算，减少 CPU 开销。

你图里写的「数据二次处理」就是它最典型业务场景：接口原始数据 → useMemo 加工成页面可用数据。

#### 基础语法:

const 处理后数据 = useMemo(计算函数, 依赖数组)

1. **第一个参数：计算函数**
   纯函数，无入参，必须返回加工后的新数据（数组 / 对象 / 数字等），用于写数据二次处理逻辑。
2. **第二个参数：依赖数组**
   放原始数据源 / 参与计算的变量；只有数组内任意值变化，才会重新执行处理函数；不变则复用缓存结果。

#### 核心作用（数据二次处理场景）

1. **避免重复计算**
   组件因无关 state 频繁重渲染时，大数据过滤、排序、循环映射不会反复执行。
2. **稳定数据引用**
   每次渲染不生成全新数组 / 对象，搭配 `React.memo` 可阻止子组件无效重渲染。
3. **大数据高性能处理**
   上万条列表筛选、分组、统计、格式化等重度数据操作必备。
...
![1784352996309](image/write/1784352996309.png)
